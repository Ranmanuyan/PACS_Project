...
* @Last Modified time: 2022-06-29 11:45:10
*/
...
double Scheduled::cdfCal2( MatrixXd A,  VectorXd v, double x1){
    int n = A.cols();
    int k = n;  // set the size of compact matrix, it should be [A.cols()/3 - A.cols()/2]
    if(n>100){
        k = n/2;
    }
    ...
    // VectorXd e1 = VectorXd::Zero(k);  // is this ok or not?
    e1.setZero(); // correct??
    e1[0] = 1.0;
    for (int i = 1; i < k; ++i) {
        e1[i] = 0;
    ...
    double cdf2 =  1- (v1.norm() * V *(((x1 * H1).exp())* e1))(0);
...
// Calculate cdf  approach - CAM appraoch

void Scheduled::balance_matrix(Eigen::MatrixXd &A, Eigen::MatrixXd &Aprime, Eigen::MatrixXd &D) {
...
void Scheduled::calAlpha(MatrixXd A, std::vector<int> mList, std::vector<double> thetaList, int &sNum, int &numM){
...
void Scheduled::sANDm(int &sNum, int &numM, MatrixXd A){
...
double Scheduled::CAM_algo(MatrixXd &A, VectorXd &v, double t){
...
double Scheduled::cdfCal3( MatrixXd A,  VectorXd v, double x1){
    int n = A.cols();
    VectorXd w1 = VectorXd::Ones(n);
    double cdf = CAM_algo(A,  v, x1);
    return cdf;
}
